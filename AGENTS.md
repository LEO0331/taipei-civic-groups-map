...
- Before an intentional fetch, capture the reviewed raw CSV contract with `npm run data:schema:capture` and the raw content baseline with `npm run data:release:capture`; after the fetch, run `npm run data:schema:check`, `npm run data:release:changes`, and `npm run data:quality:check` before conversion. Pages enforces this automatically. Do not bypass schema or semantic data-quality blockers without reviewing the upstream source and matching converter/tests; row-count collapse/spike findings are warnings only.
