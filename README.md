...
The post-demo Batches 01–21 are complete. The current application baseline is **2026-09-21** commit `05c87568af360e3fdcd30a07f4c1bbdb4e7bf934` (PR #36), after UI-family normalization, deterministic visual baselines, bundle reduction, CI runtime maintenance, source-date metadata batches A–B, and catalogue-search tuning. Frontend CI run `35578367111` passed typecheck, **141 unit tests**, production build, and the full desktop/mobile Playwright suite with **153 passed / 1 expected skip / 0 failed**. GitHub Pages run `35579373983` then repeated fresh fetch/conversion plus the full verification path and deployed the exact `05c87568` application baseline successfully.

Checked-in Data Trust evidence covers **117** static dataset directories: **53** have a readable source date, **64** have an unknown date, and **0** use a reused-snapshot fallback. The main production entry is **399.63 kB minified / 116.04 kB gzip**, below Vite's 500 kB advisory threshold. See [Post-demo verification — 2026-09-21](docs/post-demo-verification-2026-09-21.md) for the consolidated release record and [UI family classification](docs/ui-family-classification.md) for the six-family inventory.
