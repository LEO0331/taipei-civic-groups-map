...
It also includes a local-static-data directory for Taipei visually impaired massage establishments, with annual and district summaries, filtering, CSV download, and external address lookup only; it does not assert current operation, appointments, service quality, or medical effects.

The dashboard also includes a local-static-data directory for culture and arts foundations registered with Taipei Department of Cultural Affairs, with registered-address labels, filtering, CSV download, and external address lookup only; it does not assert current operation, registration, financial condition, or recommendation.

It includes a local-static-data directory for psychiatric rehabilitation and psychiatric nursing institutions, with district and source-type summaries, filtering, CSV download, and external address lookup only; it does not assert capacity, eligibility, service availability, medical quality, or recommendation.

The dashboard includes Taipei Government Application Services as a local-static-data directory with filtering, CSV download, safe external official-service links, and no application, eligibility, processing-time, or approval guarantees.

It includes senior service site courses as a local-static-data directory with course-date, capacity, location, instructor, and phone source fields; course status is relative to the data-build date and does not confirm registration or availability.

It includes rabies vaccination veterinary clinics as a local-static-data directory with district and postal-code summaries, filtering, CSV download, and external address lookup only; it does not assert vaccine stock, appointments, eligibility, service availability, or recommendation.

It includes five-cancer screening providers as a local-static-data directory with district and confirmed-service summaries, filtering, CSV download, and external address lookup only; it does not assert screening availability, appointments, eligibility, subsidy eligibility, medical quality, diagnosis, or recommendation.

It includes fertility-subsidy contracted hospitals as a local-static-data directory with district and confirmed-service summaries, filtering, and CSV download; the source has no addresses or coordinates, so it makes no map, availability, eligibility, or medical claims.

It includes senior-care institution evaluations as historical 2015–2019 records with district and result summaries, filtering, CSV download, and external address lookup only; they do not indicate current grades, operation, capacity, care quality, or recommendation.

It includes water-pipe installation contractors as a local-static-data directory with validated source-coordinate mapping, filtering, and CSV download; it does not indicate current operation, licence validity, work quality, safety, price, service area, or recommendation.

It includes pest-control businesses as a local-static-data directory with licence-record preservation, district summaries, filtering, CSV download, and external address lookup only; it does not indicate current operation, licence validity, chemical safety, treatment effectiveness, or recommendation.

It includes adult influenza vaccine providers as a local-static-data directory with district summaries, filtering, CSV download, and external address lookup only; it does not indicate vaccine stock, appointments, eligibility, service availability, or recommendation.

Hospital hemodialysis resources are available as an address-only public directory with district summaries and external map lookups. The module deliberately does not show exact markers or assert real-time capacity, eligibility, or clinical availability.
...
Industry, factories, enterprise headquarters, companies, and business public records: registered factory distribution, enterprise headquarters distribution, and Dawannan Industrial Area company directory / 產業、工廠、企業總部、公司與商業公開資料：登記工廠分布圖及基本資料、企業營運總部分布圖、大彎南段工業區廠商名錄
...
The annual evaluation module is intentionally separate from the current licensed-business directory dataset. Its `latest-year-records.json` is only the most recent evaluation resource, not a claim about current license validity or operating status; current-directory records should remain a separately sourced module when added.
...
## Additional module: Sheltered Workshop Directory / 庇護工場名冊
...
## Additional module: Occupational Safety and Health Act Violation Records / 職業安全衛生法違規公布紀錄

The separate `occupational_safety_health_violation_records` module uses UTF-8-SIG with a Big5/CP950 fallback. It parses ROC dates such as `1150605`, preserves penalty document numbers as text, keeps business / organization and responsible-person source fields, extracts article tokens such as `第6條`, and derives broad non-official content categories for filtering.

The dataset has no address, district, or coordinates, so it has no map layer. It is a labor law administrative publication explorer, not a workplace hazard map, employer blacklist, current-compliance assessment, credit signal, investment/procurement/employment advice, or legal-advice tool.
...
- `public/data/registered-recycling-business-organizations.json`
...
- `public/data/dawannan-industrial-area-company-directory.json`
- `public/data/dawannan-industrial-area-company-summary.json`
...
Recycling business organization data can be loaded from the uploaded CSV or an official resource:

```bash
npm run data:fetch:recycling-organizations -- --force --local=/absolute/path/to/臺北市回收業機構名冊.csv
npm run data:convert:recycling-organizations
tsx scripts/buildPublicRecordsSummary.ts
```

Registered factory data can be loaded from the uploaded CSV or an official resource:

```bash
npm run data:fetch:registered-factories -- --force --local=/absolute/path/to/臺北市登記工廠分布圖及基本資料.csv
npm run data:convert:registered-factories
tsx scripts/buildPublicRecordsSummary.ts
```
...
Sheltered workshop data can be loaded from the uploaded CSV or an official resource:
...
npm run data:fetch:dawannan-companies -- --force --local=/absolute/path/to/臺北市大彎南段工業區廠商名錄.csv
npm run data:convert:dawannan-companies
tsx scripts/buildPublicRecordsSummary.ts
```
...
Animal medicine seller data can be loaded from the uploaded CSV or an official resource:
...
Occupational Safety and Health Act violation records can be loaded from the uploaded CSV or an official resource:
...
Licensed electronic game arcade operator records can be loaded from the uploaded CSV or an official resource:

```bash
npm run data:fetch:licensed-arcades -- --force --local=/absolute/path/to/臺北市登記有案電子遊戲場業列管清冊-11409.csv
npm run data:convert:licensed-arcades
tsx scripts/buildPublicRecordsSummary.ts
```
...
## Alternative Service Reserve Statistics
...
## Influenza Vaccine Providers for Children Age 3+

The `influenza_vaccine_providers_children_3plus` module is a separate local static directory sourced from [臺北市流感疫苗合約醫療院所名冊（3歲以上（含3歲）幼童）](https://data.taipei/dataset/detail?id=f2a66aec-9b15-4580-9d04-9e991d45e908). It preserves all six source fields, raw district codes, mapped display districts, voice-reservation text and normalized state, CSV and metadata update dates, and local ingestion metadata. It supports district, voice-reservation, phone, address, institution, keyword, sorting, pagination, source details, telephone/copy actions, external map lookup, and filtered CSV export. It does not establish current vaccine stock, appointment availability, same-day vaccination, eligibility, vaccine brand, subsidy, quality, or recommendation.
