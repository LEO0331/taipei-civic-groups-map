...
const rawDirectory = join(process.cwd(), 'data/raw/general-chinese-medicine-institutions');
const outputDirectory = join(process.cwd(), 'public/data/general-chinese-medicine-institutions');
...
const duplicates: string[] = [], missingNames: string[] = [], missingAddresses: string[] = [], malformedPhones: string[] = [], unknownPostalCodes: string[] = [], unresolvedDistricts: string[] = [];
const seen = new Set<string>();
const records = rows.flatMap((row) => {
  const [sourceSequenceNumber = '', institutionName = '', postalCode = '', address = '', phone = ''] = row.map(clean);
  const id = sourceSequenceNumber || `${institutionName}|${address}`, key = `${institutionName}|${address}|${phone}`;
  ...
  seen.add(key);
  if (!institutionName) missingNames.push(id); if (!address) missingAddresses.push(id);
  if (phone && !/^[+()（）\-\s\d#xXext.、,;/及與]+$/.test(phone)) malformedPhones.push(id);
  const addressDistrict = address.match(/[\u4e00-\u9fff]{2}\u5340/)?.[0] ?? '', postalDistrict = postalDistricts[postalCode] ?? '';
...
await writeFile(join(outputDirectory, 'conversion-report.json'), JSON.stringify({ source: '臺北市中醫一般科醫療機構', encoding, sourceFields: headers.map(clean), inputRows: rows.length, recordCount: records.length, duplicates, missingNames, missingAddresses, malformedPhones, unknownPostalCodes: [...new Set(unknownPostalCodes)], unresolvedDistricts }, null, 2));
console.log(`Converted ${records.length} general Chinese-medicine institution records (${encoding}).`);
