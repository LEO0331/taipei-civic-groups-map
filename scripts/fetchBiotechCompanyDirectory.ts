...
const datasetId = 'a05ee8ee-d7f1-4024-86c1-e2f97f2120bf';
const rawDir = join(process.cwd(), 'data/raw/biotech-company-directory');
const destination = join(rawDir, 'biotech-company-directory.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市生技廠商企業名錄', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', rowCount: undefined, notes, failure, previous }, null, 2));
}
...
console.log(`Prepared biotech company directory CSV at ${destination}.`);
