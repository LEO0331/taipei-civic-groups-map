...
const datasetId = '3cc250f5-9f5a-4670-ac7b-f13ecd316032';
const rawDir = join(process.cwd(), 'data/raw/child-medical-subsidy-contracted-providers');
const destination = join(rawDir, 'child-medical-subsidy-contracted-providers.csv');
const metadataPath = join(rawDir, 'fetch-metadata.json');
const sourcePage = `https://data.taipei/dataset/detail?id=${datasetId}`;
const defaultLocal = '/Users/Leo/Downloads/臺北市兒童醫療補助特約院所名冊(公告V1150601)-更新資料平台.csv';
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市兒童醫療補助特約院所名冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared child medical subsidy provider CSV at ${destination}.`);
