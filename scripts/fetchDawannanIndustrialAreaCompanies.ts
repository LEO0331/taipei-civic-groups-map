...
const datasetId = 'dd950d9c-01c0-452d-b8d0-f9cf38f3a55d';
const rawDir = join(process.cwd(), 'data/raw/dawannan-industrial-area-company-directory');
const destination = join(rawDir, 'dawannan-industrial-area-company-directory.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市大彎南段工業區廠商名錄', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared Dawannan Industrial Area company CSV at ${destination}.`);
