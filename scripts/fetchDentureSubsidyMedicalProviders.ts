...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '假牙補助醫療院所名單', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'Big5 / CP950 with UTF-8-SIG fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared denture subsidy provider CSV at ${destination}.`);
