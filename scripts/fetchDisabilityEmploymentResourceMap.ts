...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市身障就業資源地圖', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'Big5 / CP950 with UTF-8-SIG fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared disability employment resource CSV at ${destination}.`);
