...
const datasetId = 'd455b149-1a2f-4d5a-a9a8-315eb71f51f6';
const rawDir = join(process.cwd(), 'data/raw/elderly-welfare-institutions');
const destination = join(rawDir, 'elderly-welfare-institutions.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市老人福利機構名冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared elderly welfare institution CSV at ${destination}.`);
