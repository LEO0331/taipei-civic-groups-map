...
const datasetId = 'e39d8ba6-999f-48bb-a8ad-c3c27ee90c78';
const rawDir = join(process.cwd(), 'data/raw/employment-agency-intermediary-companies');
const destination = join(rawDir, 'employment-agency-intermediary-companies.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市仲介公司資料', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG / UTF-8 or CP950 / Big5-compatible', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared employment agency intermediary companies CSV at ${destination}.`);
