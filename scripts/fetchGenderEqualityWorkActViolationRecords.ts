...
const datasetId = '12f3421a-94f4-4a5e-8642-143dee2fa551';
const rawDir = join(process.cwd(), 'data/raw/gender-equality-work-act-violation-records');
const destination = join(rawDir, 'gender-equality-work-act-violation-records.csv');
const metadataPath = join(rawDir, 'fetch-metadata.json');
const sourcePage = `https://data.taipei/dataset/detail?id=${datasetId}`;
const defaultLocal = '/Users/Leo/Downloads/臺北市政府勞動局違反性別平等工作法事業單位及事業主公布總表【公告月份：11506】.csv';
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市政府勞動局違反性別平等工作法事業單位及事業主公布總表', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'Big5 / CP950 with UTF-8-SIG fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared Gender Equality Work Act violation CSV at ${destination}.`);
