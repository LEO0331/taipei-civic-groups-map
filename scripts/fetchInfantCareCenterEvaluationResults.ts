...
const datasetId = 'e7b45593-9d44-469c-97fa-f1a52c69ebaa';
const rawDir = join(process.cwd(), 'data/raw/infant-care-center-evaluation-results');
const destination = join(rawDir, 'infant-care-center-evaluation-results.csv');
const metadataPath = join(rawDir, 'fetch-metadata.json');
const sourcePage = `https://data.taipei/dataset/detail?id=${datasetId}`;
const args = process.argv.slice(2), local = args.find((arg) => arg.startsWith('--local='))?.slice(8) ?? '/Users/Leo/Downloads/臺北市托嬰中心評鑑結果.csv', url = args.find((arg) => arg.startsWith('--url='))?.slice(6), force = args.includes('--force');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市托嬰中心評鑑結果', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', rowCount: undefined, notes, failure, previous }, null, 2));
}
...
try {
  ...
  await metadata('local-file', [`Copied from ${basename(local)}`]);
} catch {
  try {
    ...
    await metadata(resolved, [url ? 'Official resource URL supplied with --url.' : 'Official resource discovered from dataset page.']);
  } catch (error) {
...
console.log(`Prepared infant care center evaluation CSV at ${destination}.`);
