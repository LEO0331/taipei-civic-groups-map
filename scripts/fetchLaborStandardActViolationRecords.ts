...
const datasetId = '23630879-4926-4877-a48a-a0ae6cc2f7d5';
const rawDir = join(process.cwd(), 'data/raw/labor-standard-act-violation-records');
const destination = join(rawDir, 'labor-standard-act-violation-records.csv');
...
async function writeMetadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({
    sourceUrl, resourceName: '臺北市政府勞動局違反勞動基準法事業單位及事業主公布總表',
    downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', notes, failure, previous,
  }, null, 2));
}
...
if (local) {
  ...
  console.log(`Downloaded labor violation CSV to ${destination}.`);
} else {
  try {
