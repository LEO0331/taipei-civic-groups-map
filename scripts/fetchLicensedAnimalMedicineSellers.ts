...
const datasetId = '9100b52e-f939-492f-9a8f-ba0adbd304ce';
const rawDir = join(process.cwd(), 'data/raw/licensed-animal-medicine-sellers');
const destination = join(rawDir, 'licensed-animal-medicine-sellers.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '提供動物用藥品販賣業者名冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'CP950 / Big5-compatible or UTF-8-SIG', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared licensed animal medicine sellers CSV at ${destination}.`);
