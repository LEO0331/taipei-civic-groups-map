...
const datasetId = '20774fbb-5671-4850-b307-af6b5976077d';
const rawDir = join(process.cwd(), 'data/raw/licensed-electronic-game-arcade-operators');
const destination = join(rawDir, 'licensed-electronic-game-arcade-operators.csv');
const metadataPath = join(rawDir, 'fetch-metadata.json');
const sourcePage = `https://data.taipei/dataset/detail?id=${datasetId}`;
const defaultLocal = '/Users/Leo/Downloads/臺北市登記有案電子遊戲場業列管清冊-11409.csv';
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市合法電子遊戲場業者清冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 / CP950 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared licensed electronic game arcade operator CSV at ${destination}.`);
