...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市合法八大行業業者清冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 / CP950 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared licensed special entertainment business operator CSV at ${destination}.`);
