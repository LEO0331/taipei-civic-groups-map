...
const datasetId = 'f56e77c6-cc69-480c-8ba4-057fc7e1d8d6';
const rawDir = join(process.cwd(), 'data/raw/performing-arts-groups');
const destination = join(rawDir, 'performing-arts-groups.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市演藝團體名冊', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared performing-arts group CSV at ${destination}.`);
