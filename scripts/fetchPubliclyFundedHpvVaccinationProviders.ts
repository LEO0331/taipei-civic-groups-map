...
const datasetId = '96f143fe-4c95-4d88-9985-77f28e2d2c3d';
const rawDir = join(process.cwd(), 'data/raw/publicly-funded-hpv-vaccination-providers');
const destination = join(rawDir, 'publicly-funded-hpv-vaccination-providers.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市公費HPV疫苗特約醫療院所', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', rowCount: undefined, notes, failure, previous }, null, 2));
}
...
console.log(`Prepared publicly funded HPV vaccination provider CSV at ${destination}.`);
