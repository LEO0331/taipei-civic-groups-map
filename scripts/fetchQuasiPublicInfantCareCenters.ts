...
const datasetId = 'aeaaa517-089c-42a7-ad5b-60fef89c3545';
const rawDir = join(process.cwd(), 'data/raw/quasi-public-infant-care-centers');
const destination = join(rawDir, 'quasi-public-infant-care-centers.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市準公共化托嬰中心', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared quasi-public infant care center CSV at ${destination}.`);
