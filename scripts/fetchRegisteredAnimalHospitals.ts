...
const datasetId = '01bcb5ee-7c18-41fa-86d4-4e75daee1f94';
const rawDir = join(process.cwd(), 'data/raw/registered-animal-hospitals');
const destination = join(rawDir, 'registered-animal-hospitals.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市動物醫院一覽表', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared registered animal hospital CSV at ${destination}.`);
