...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市登記工廠分布圖及基本資料', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG with Big5 / CP950 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared registered factory distribution CSV at ${destination}.`);
