...
const datasetId = '4d7d0b46-2e90-4ee7-b000-c0f2f3a37651';
const rawDir = join(process.cwd(), 'data/raw/registered-hotels');
const destination = join(rawDir, 'registered-hotels.csv');
...
async function writeMetadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({
    sourceUrl, resourceName: '臺北市一般旅館名冊', downloadedAt: new Date().toISOString(),
...
if (local) {
  ...
  console.log(`Downloaded registered hotel CSV to ${destination}.`);
} else {
  try {
