...
const datasetId = 'bea69229-8349-4208-8a68-988718f4ea48';
const rawDir = join(process.cwd(), 'data/raw/registered-labor-unions');
const destination = join(rawDir, 'registered-labor-unions.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({
    sourceUrl, resourceName: '臺北市各工會名單及聯絡方式', downloadedAt: new Date().toISOString(),
    fileSize: file.size, encoding: 'CP950 / Big5-compatible', notes, failure, previous,
  }, null, 2));
}
...
console.log(`Prepared registered labor union CSV at ${destination}.`);
