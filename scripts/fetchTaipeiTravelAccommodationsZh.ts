...
const datasetId = '58093ba6-4c98-4148-b27a-50ad97d7afca';
const rawDir = join(process.cwd(), 'data/raw/taipei-travel-accommodations-zh');
const destination = join(rawDir, 'taipei-travel-accommodations-zh.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市臺北旅遊網住宿資料(中文)', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'UTF-8-SIG', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared Taipei Travel accommodation CSV at ${destination}.`);
