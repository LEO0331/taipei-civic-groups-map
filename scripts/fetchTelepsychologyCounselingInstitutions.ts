...
const datasetId = '428a78d5-867a-4e55-9630-040a89c8cd94';
const rawDir = join(process.cwd(), 'data/raw/telepsychology-counseling-institutions');
const destination = join(rawDir, 'telepsychology-counseling-institutions.csv');
...
async function metadata(sourceUrl: string, notes: string[], failure?: string) {
  ...
  await writeFile(metadataPath, JSON.stringify({ sourceUrl, resourceName: '臺北市可執行通訊心理諮商之心理機構', downloadedAt: new Date().toISOString(), fileSize: file.size, encoding: 'CP950 / Big5-compatible with UTF-8 fallback at conversion', notes, failure, previous }, null, 2));
}
...
console.log(`Prepared telepsychology counseling institution CSV at ${destination}.`);
