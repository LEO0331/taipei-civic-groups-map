...
export default function App() {
  ...
  const [postpartumRecords, setPostpartumRecords] = useState<any[]>([]); const [outCityFuneralRecords, setOutCityFuneralRecords] = useState<any[]>([]); const [hotelHygieneRecords, setHotelHygieneRecords] = useState<any[]>([]); const [kindergartenRecords, setKindergartenRecords] = useState<any[]>([]);
  ...
  useEffect(() => { const j=(p:string)=>fetch(`${import.meta.env.BASE_URL}${p}`).then(r=>{if(!r.ok)throw Error(p);return r.json()}); Promise.all([j('data/registered-postpartum-care-institutions/records.json'),j('data/out-of-city-funeral-service-businesses/records.json'),j('data/hotel-hygiene-certification-directory/records.json'),j('data/kindergarten-basic-evaluation-pass-records/records.json')]).then(([a,b,c,d])=>{setPostpartumRecords(a);setOutCityFuneralRecords(b);setHotelHygieneRecords(c);setKindergartenRecords(d)}).catch(()=>setLoadError(true)); }, []);
  ...
  tabs.splice(7,0,['postpartumCareInstitutions',language==='zh'?'產後護理機構':'Postpartum Care Institutions'],['outCityFuneralBusinesses',language==='zh'?'外縣市殯葬業者':'Out-of-City Funeral Businesses'],['hotelHygieneDirectory',language==='zh'?'旅館衛生認證':'Hotel Hygiene Certification'],['kindergartenEvaluationPass',language==='zh'?'幼兒園評鑑通過名單':'Kindergarten Evaluation Pass Records']);
