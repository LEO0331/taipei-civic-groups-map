...
import type {
  ...
  ContractedVaccinationMedicalProviderRecord, ContractedVaccinationMedicalProviderSummary, TelepsychologyCounselingInstitutionRecord, TelepsychologyCounselingInstitutionSummary,
} from './types';

const copy = {
  zh: {
    title: '台北公共登記與行政紀錄地圖', subtitle: '人民團體、演藝團體、工會、醫療與心理健康院所、立案機構、旅宿、旅遊住宿、採購、補助、法規、產業園區、動物照護與兒童照護公開紀錄探索',
    civicGroups: '人民團體', performingArtsGroups: '演藝團體', vaccinationProviders: '預防接種院所', telepsychology: '通訊心理諮商', laborUnions: '工會名單', infantCareCenters: '準公共化托嬰中心', travelAccommodations: '旅遊住宿', industryGrants: '產業補助廠商', metroProcurement: '捷運採購時程', registeredCramSchools: '立案補習班', registeredHotels: '一般旅館名冊', laborViolations: '勞基法違規公布紀錄', nangangCompanies: '南港軟體工業園區廠商', animalHospitals: '動物醫院一覽表', comparison: '行政區比較',
    ...
    footer: '資料來源：臺北市人民團體名冊、臺北市演藝團體名冊、臺北市各工會名單及聯絡方式、各項預防接種合約醫療院所、臺北市可執行通訊心理諮商之心理機構、產業補助、捷運採購、立案補習班、一般旅館、臺北旅遊網住宿資料、勞基法違規公布、南港軟體工業園區廠商、臺北市動物醫院一覽表與臺北市準公共化托嬰中心等公開資料。各資料集性質不同，最新與正式資訊請以主管機關正式公告及官方系統為準。',
  },
  en: {
    title: 'Taipei Public Records Explorer', subtitle: 'Civic groups, performing-arts groups, labor unions, healthcare and mental-health providers, registered institutions, lodging records, travel accommodations, procurement, grants, compliance, industry park, animal-care, and childcare public records explorer',
    civicGroups: 'Civic Groups', performingArtsGroups: 'Performing Arts', vaccinationProviders: 'Vaccination Providers', telepsychology: 'Telepsychology', laborUnions: 'Labor Unions', infantCareCenters: 'Quasi-Public Infant Care Centers', travelAccommodations: 'Travel Accommodations', industryGrants: 'Industry Grant Recipients', metroProcurement: 'Metro Procurement Schedule', registeredCramSchools: 'Registered Cram Schools', registeredHotels: 'Registered Hotels', laborViolations: 'Labor Standards Act Violation Records', nangangCompanies: 'Nangang Software Park Companies', animalHospitals: 'Registered Animal Hospitals', comparison: 'District Comparison',
...
function CombinedOverview({ civic, performingArts, vaccinationProviders, telepsychology, laborUnions, infantCare, travelAccommodations, grants, procurement, cramSchools, hotels, laborViolations, nangangCompanies, animalHospitals, language }: {
  civic: CivicGroupSummary; performingArts: PerformingArtsGroupSummary; vaccinationProviders: ContractedVaccinationMedicalProviderSummary; telepsychology: TelepsychologyCounselingInstitutionSummary; laborUnions: RegisteredLaborUnionSummary; infantCare: QuasiPublicInfantCareCenterSummary; travelAccommodations: TaipeiTravelAccommodationZhSummary; grants: IndustryGrantSummary; procurement: MetroProcurementScheduleSummary; cramSchools: RegisteredCramSchoolSummary; hotels: RegisteredHotelSummary; laborViolations: LaborStandardActViolationSummary; nangangCompanies: NangangSoftwareParkCompanySummary; animalHospitals: RegisteredAnimalHospitalSummary; language: Language;
}) {
...
export default function App() {
  ...
  const [tab, setTab] = useState<'civic' | 'performingArts' | 'vaccinationProviders' | 'telepsychology' | 'laborUnions' | 'infantCare' | 'travelAccommodations' | 'grants' | 'procurement' | 'cramSchools' | 'hotels' | 'laborViolations' | 'nangangCompanies' | 'animalHospitals' | 'comparison' | 'overview' | 'notes'>('civic');
  ...
  const tabs = [
    ['civic', t.civicGroups], ['performingArts', t.performingArtsGroups], ['vaccinationProviders', t.vaccinationProviders], ['telepsychology', t.telepsychology], ['laborUnions', t.laborUnions], ['infantCare', t.infantCareCenters], ['travelAccommodations', t.travelAccommodations], ['grants', t.industryGrants], ['procurement', t.metroProcurement],
  ...
  return <div className="app">
    ...
    <main>
      ...
      {tab === 'overview' && summary && performingArtsSummary && vaccinationProviderSummary && telepsychologySummary && laborUnionSummary && infantCareSummary && travelAccommodationSummary && grantSummary && procurementSummary && cramSchoolSummary && hotelSummary && laborViolationSummary && nangangCompanySummary && animalHospitalSummary && <CombinedOverview civic={summary} performingArts={performingArtsSummary} vaccinationProviders={vaccinationProviderSummary} telepsychology={telepsychologySummary} laborUnions={laborUnionSummary} infantCare={infantCareSummary} travelAccommodations={travelAccommodationSummary} grants={grantSummary} procurement={procurementSummary} cramSchools={cramSchoolSummary} hotels={hotelSummary} laborViolations={laborViolationSummary} nangangCompanies={nangangCompanySummary} animalHospitals={animalHospitalSummary} language={language} />}
      {tab === 'notes' && <section className="workspace notes"><div className="section-heading"><p>09 / METHODOLOGY</p><h2>{t.notes}</h2></div>
        <blockquote>{language === 'zh' ? '本網站整理臺北市公開資料中的人民團體名冊、演藝團體名冊、各工會名單及聯絡方式、各項預防接種合約醫療院所、可執行通訊心理諮商之心理機構、產業補助廠商資料、捷運採購案件預定招標時程、立案補習班資訊、一般旅館名冊、臺北旅遊網住宿資料、勞基法違規公布紀錄、南港軟體工業園區廠商資料、動物醫院一覽表、準公共化托嬰中心等公開資料，僅供資料探索與整理使用。各資料集性質不同，不應直接解讀為相同類型組織或活動。醫療、心理健康與預防接種相關資料不代表即時門診時間、即時預約名額、疫苗庫存、接種資格、醫療建議、心理治療建議、危機處理服務、院所品質、推薦排名或官方背書。' : 'This site organizes Taipei public-data records such as civic group directory records, performing-arts group registry records, registered labor union records, contracted vaccination medical provider records, telepsychology counseling institution records, industry grant recipient records, Taipei Metro planned procurement tender schedules, registered cram-school records, registered hotel records, Taipei Travel accommodation records, Labor Standards Act violation publication records, Nangang Software Park company records, animal hospital directory records, quasi-public infant care center records, and related public records for data exploration and organization only. These datasets have different meanings and should not be interpreted as the same type of organization or activity. Healthcare, mental-health, and vaccination-related data does not represent real-time clinic hours, real-time appointment availability, vaccine stock, vaccination eligibility, medical advice, psychotherapy advice, crisis intervention service, provider quality, recommendation ranking, or official endorsement.'}</blockquote>
