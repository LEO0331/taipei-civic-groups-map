...
const copy = {
  zh: {
    title: '台北公共登記與行政紀錄地圖', subtitle: '人民團體、演藝團體、工會、立案機構、旅宿、旅遊住宿、採購、補助、法規、產業園區、動物照護與兒童照護公開紀錄探索',
    civicGroups: '人民團體', performingArtsGroups: '演藝團體', laborUnions: '工會名單', infantCareCenters: '準公共化托嬰中心', travelAccommodations: '旅遊住宿', industryGrants: '產業補助廠商', metroProcurement: '捷運採購時程', registeredCramSchools: '立案補習班', registeredHotels: '一般旅館名冊', laborViolations: '勞基法違規公布紀錄', nangangCompanies: '南港軟體工業園區廠商', animalHospitals: '動物醫院一覽表', comparison: '行政區比較',
    ...
    footer: '資料來源：臺北市人民團體名冊、臺北市演藝團體名冊、臺北市各工會名單及聯絡方式、產業補助、捷運採購、立案補習班、一般旅館、臺北旅遊網住宿資料、勞基法違規公布、南港軟體工業園區廠商、臺北市動物醫院一覽表與臺北市準公共化托嬰中心等公開資料。各資料集性質不同，最新與正式資訊請以主管機關正式公告及官方系統為準。',
  },
  en: {
    title: 'Taipei Public Records Explorer', subtitle: 'Civic groups, performing-arts groups, labor unions, registered institutions, lodging records, travel accommodations, procurement, grants, compliance, industry park, animal-care, and childcare public records explorer',
    civicGroups: 'Civic Groups', performingArtsGroups: 'Performing Arts', laborUnions: 'Labor Unions', infantCareCenters: 'Quasi-Public Infant Care Centers', travelAccommodations: 'Travel Accommodations', industryGrants: 'Industry Grant Recipients', metroProcurement: 'Metro Procurement Schedule', registeredCramSchools: 'Registered Cram Schools', registeredHotels: 'Registered Hotels', laborViolations: 'Labor Standards Act Violation Records', nangangCompanies: 'Nangang Software Park Companies', animalHospitals: 'Registered Animal Hospitals', comparison: 'District Comparison',
    ...
    footer: 'Data sources: Taipei civic groups, Taipei performing-arts group registry dataset, registered labor unions, industry grants, Metro procurement, registered cram schools, registered hotels, Taipei Travel accommodation dataset, Labor Standards Act violation publication records, Nangang Software Park company directory, Taipei animal hospital directory, Taipei quasi-public infant care center records, and related public data. These datasets have different meanings. Latest official information should be verified with authorities and official systems.',
  },
} as const;
...
function CombinedOverview({ civic, performingArts, laborUnions, infantCare, travelAccommodations, grants, procurement, cramSchools, hotels, laborViolations, nangangCompanies, animalHospitals, language }: {
  civic: CivicGroupSummary; performingArts: PerformingArtsGroupSummary; laborUnions: RegisteredLaborUnionSummary; infantCare: QuasiPublicInfantCareCenterSummary; travelAccommodations: TaipeiTravelAccommodationZhSummary; grants: IndustryGrantSummary; procurement: MetroProcurementScheduleSummary; cramSchools: RegisteredCramSchoolSummary; hotels: RegisteredHotelSummary; laborViolations: LaborStandardActViolationSummary; nangangCompanies: NangangSoftwareParkCompanySummary; animalHospitals: RegisteredAnimalHospitalSummary; language: Language;
}) {
...
export default function App() {
  ...
  const [tab, setTab] = useState<'civic' | 'performingArts' | 'laborUnions' | 'infantCare' | 'travelAccommodations' | 'grants' | 'procurement' | 'cramSchools' | 'hotels' | 'laborViolations' | 'nangangCompanies' | 'animalHospitals' | 'comparison' | 'overview' | 'notes'>('civic');
  ...
  const [report, setReport] = useState<{
    convertedAt?: string; performingArtsGroups?: { convertedAt?: string }; registeredLaborUnions?: { convertedAt?: string }; quasiPublicInfantCareCenters?: { convertedAt?: string }; taipeiTravelAccommodationsZh?: { convertedAt?: string }; industryGrantRecipients?: { convertedAt?: string }; metroProcurementSchedules?: { convertedAt?: string }; registeredCramSchools?: { convertedAt?: string }; registeredHotels?: { convertedAt?: string }; laborStandardActViolationRecords?: { convertedAt?: string }; nangangSoftwareParkCompanies?: { convertedAt?: string }; registeredAnimalHospitals?: { convertedAt?: string };
  }>({});
  ...
  useEffect(() => {
    ...
    ]).then(([groupData, summaryData, performingArtsData, performingArtsSummaryData, laborUnionData, laborUnionSummaryData, infantCareData, infantCareSummaryData, travelData, travelSummaryData, grantData, grantSummaryData, procurementData, procurementSummaryData, cramSchoolData, cramSchoolSummaryData, hotelData, hotelSummaryData, laborSummaryData, laborManifestData, nangangData, nangangSummaryData, animalData, animalSummaryData, reportData]) => {
  ...
  const tabs = [
    ['civic', t.civicGroups], ['performingArts', t.performingArtsGroups], ['laborUnions', t.laborUnions], ['infantCare', t.infantCareCenters], ['travelAccommodations', t.travelAccommodations], ['grants', t.industryGrants], ['procurement', t.metroProcurement],
  ...
  return <div className="app">
    ...
    <main>
      ...
      {!loadError && (!summary || !performingArtsSummary || !laborUnionSummary || !infantCareSummary || !travelAccommodationSummary || !grantSummary || !procurementSummary || !cramSchoolSummary || !hotelSummary || !laborViolationSummary || !laborViolationManifest || !nangangCompanySummary || !animalHospitalSummary) && <p className="status" role="status">{t.loading}</p>}
      ...
      {tab === 'overview' && summary && performingArtsSummary && laborUnionSummary && infantCareSummary && travelAccommodationSummary && grantSummary && procurementSummary && cramSchoolSummary && hotelSummary && laborViolationSummary && nangangCompanySummary && animalHospitalSummary && <CombinedOverview civic={summary} performingArts={performingArtsSummary} laborUnions={laborUnionSummary} infantCare={infantCareSummary} travelAccommodations={travelAccommodationSummary} grants={grantSummary} procurement={procurementSummary} cramSchools={cramSchoolSummary} hotels={hotelSummary} laborViolations={laborViolationSummary} nangangCompanies={nangangCompanySummary} animalHospitals={animalHospitalSummary} language={language} />}
      {tab === 'notes' && <section className="workspace notes"><div className="section-heading"><p>09 / METHODOLOGY</p><h2>{t.notes}</h2></div>
        <blockquote>{language === 'zh' ? '本網站整理臺北市公開資料中的人民團體名冊、演藝團體名冊、各工會名單及聯絡方式、產業補助廠商資料、捷運採購案件預定招標時程、立案補習班資訊、一般旅館名冊、臺北旅遊網住宿資料、勞基法違規公布紀錄、南港軟體工業園區廠商資料、動物醫院一覽表、準公共化托嬰中心等公開資料，僅供資料探索與整理使用。各資料集性質不同，不應直接解讀為相同類型組織或活動。演藝團體資料不代表即時營運狀態、活動檔期、演出品質、推薦排名、售票資訊、補助狀態、法律意見或官方背書。' : 'This site organizes Taipei public-data records such as civic group directory records, performing-arts group registry records, registered labor union records, industry grant recipient records, Taipei Metro planned procurement tender schedules, registered cram-school records, registered hotel records, Taipei Travel accommodation records, Labor Standards Act violation publication records, Nangang Software Park company records, animal hospital directory records, quasi-public infant care center records, and related public records for data exploration and organization only. These datasets have different meanings and should not be interpreted as the same type of organization or activity. Performing-arts group data does not represent real-time operating status, event schedules, performance quality, recommendation ranking, ticketing information, grant status, legal advice, or official endorsement.'}</blockquote>
