...
const copy = {
  zh: {
    ...
    footer: '資料來源：臺北市人民團體名冊、臺北市演藝團體名冊、臺北市各工會名單及聯絡方式、臺北市身障就業資源地圖、臺北市庇護工場名冊、臺北市政府警察局當舖業資料清冊、臺北市合法電子遊戲場業者清冊、各項預防接種合約醫療院所、臺北市公費HPV疫苗特約醫療院所、臺北市兒童醫療補助特約院所名冊、臺北市假牙補助醫療院所名單、臺北市可執行通訊心理諮商之心理機構、臺北市營業場所投保公共意外險清冊、商業設立變更歇業登記異動資料、臺北市核准公司設立變更解散清冊、產業補助、臺北市生技廠商企業名錄、捷運採購、立案補習班、一般旅館、臺北旅遊網住宿資料、勞基法違規公布、職安法違規公布、性別平等工作法違規公布、臺北市消費爭議無故不到場協商之被申訴企業經營者列表、南港軟體工業園區廠商、臺北市大彎南段工業區廠商名錄、臺北市動物醫院一覽表、臺北市準公共化托嬰中心、臺北市托嬰中心評鑑結果與臺北市老人福利機構名冊等公開資料。各資料集性質不同，最新與正式資訊請以主管機關正式公告及官方系統為準。',
  },
  en: {
...
export default function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {tab === 'overview' && summary && performingArtsSummary && vaccinationProviderSummary && hpvProviderSummary && childMedicalSubsidyProviderSummary && dentureSubsidyProviderSummary && disabilityEmploymentResourceSummary && shelteredWorkshopSummary && licensedPawnshopSummary && licensedArcadeSummary && telepsychologySummary && publicLiabilitySummary && businessChangeSummary && companyChangeSummary && laborUnionSummary && infantCareSummary && infantCareEvaluationSummary && elderlyWelfareSummary && biotechCompanySummary && travelAccommodationSummary && grantSummary && procurementSummary && cramSchoolSummary && hotelSummary && laborViolationSummary && oshViolationSummary && genderEqualityViolationSummary && consumerDisputeSummary && nangangCompanySummary && dawannanCompanySummary && animalHospitalSummary && veterinarianSummary && <CombinedOverview civic={summary} performingArts={performingArtsSummary} vaccinationProviders={vaccinationProviderSummary} hpvProviders={hpvProviderSummary} childMedicalSubsidyProviders={childMedicalSubsidyProviderSummary} dentureSubsidyProviders={dentureSubsidyProviderSummary} disabilityEmploymentResources={disabilityEmploymentResourceSummary} shelteredWorkshops={shelteredWorkshopSummary} licensedPawnshops={licensedPawnshopSummary} licensedArcades={licensedArcadeSummary} telepsychology={telepsychologySummary} publicLiabilityInsurance={publicLiabilitySummary} businessChanges={businessChangeSummary} companyChanges={companyChangeSummary} laborUnions={laborUnionSummary} infantCare={infantCareSummary} infantCareEvaluations={infantCareEvaluationSummary} elderlyWelfare={elderlyWelfareSummary} biotechCompanies={biotechCompanySummary} travelAccommodations={travelAccommodationSummary} grants={grantSummary} procurement={procurementSummary} cramSchools={cramSchoolSummary} hotels={hotelSummary} laborViolations={laborViolationSummary} oshViolations={oshViolationSummary} genderEqualityViolations={genderEqualityViolationSummary} consumerDisputeAbsence={consumerDisputeSummary} nangangCompanies={nangangCompanySummary} dawannanCompanies={dawannanCompanySummary} animalHospitals={animalHospitalSummary} veterinarians={veterinarianSummary} language={language} />}
      {tab === 'notes' && <section className="workspace notes"><div className="section-heading"><p>09 / METHODOLOGY</p><h2>{t.notes}</h2></div>
        <blockquote>{language === 'zh' ? '本網站整理臺北市公開資料中的人民團體名冊、演藝團體名冊、工會名單、身障就業資源、庇護工場名冊、當舖業資料清冊、合法電子遊戲場業者清冊、產業補助廠商資料、生技廠商企業名錄、捷運採購案件預定招標時程、立案補習班資訊、一般旅館名冊、臺北旅遊網住宿資料、勞基法違規公布紀錄、職安法違規公布紀錄、性別平等工作法違規公布紀錄、消費爭議不到場公告、南港軟體工業園區廠商資料、大彎南段工業區廠商名錄、動物醫院一覽表、獸醫師資訊、準公共化托嬰中心、托嬰中心評鑑結果、老人福利機構名冊、各項預防接種合約醫療院所、公費HPV疫苗特約醫療院所、兒童醫療補助特約院所、假牙補助醫療院所、可執行通訊心理諮商之心理機構、營業場所投保公共意外險清冊、商業設立變更及歇業登記異動資料、公司設立變更及解散登記異動資料等公開資料，僅供資料探索與整理使用。各資料集性質不同，不應直接解讀為相同類型組織、活動、服務可用性、即時營業狀態、醫療建議、兒童照顧或老人照顧建議、就業媒合保證、補助資格判定、消費糾紛狀態、貸款建議、官方排名、法規遵循狀態、法律責任、信用狀態、投資訊號、黑名單、土地使用法律意見、法律意見、財務建議或官方背書。最新與正式資訊請以主管機關正式公告及官方系統為準。' : 'This site organizes Taipei public-data records such as civic group directory records, performing-arts group registry records, labor union directory records, disability employment resource records, sheltered workshop directory records, licensed pawnshop directory records, licensed electronic game arcade operator records, industry grant recipient records, biotech company directory records, Taipei Metro planned procurement tender schedules, registered cram-school records, registered hotel records, Taipei Travel accommodation records, Labor Standards Act violation publication records, Occupational Safety and Health Act violation publication records, Gender Equality in Employment Act violation publication records, consumer dispute absence notices, Nangang Software Park company records, Dawannan Industrial Area company directory records, animal hospital directory records, veterinarian professional registry records, quasi-public infant care center records, infant care center evaluation result records, elderly welfare institution directory records, contracted vaccination medical provider records, publicly funded HPV vaccination provider records, child medical subsidy contracted provider records, denture subsidy medical provider records, telepsychology counseling institution records, business premises public liability insurance records, business establishment / modification / closure registration change records, company establishment / modification / dissolution registration change records, and related public records for data exploration and organization only. These datasets have different meanings and should not be interpreted as the same type of organization, activity, service availability, real-time operating status, medical advice, childcare or elderly care advice, employment placement guarantee, subsidy eligibility determination, consumer dispute status, loan advice, official ranking, legal compliance status, legal liability, credit status, investment signal, blacklist, land-use legal opinion, legal advice, financial advice, or official endorsement. Latest and official information should be verified with official authority notices and official systems.'}</blockquote>
