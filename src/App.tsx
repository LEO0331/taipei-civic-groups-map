...
import type {
  ...
  BusinessPremisesPublicLiabilityInsuranceRecord, BusinessPremisesPublicLiabilityInsuranceSummary, BusinessRegistrationChangeRecord, BusinessRegistrationChangeSummary, CompanyRegistrationChangeRecord, CompanyRegistrationChangeSummary, ContractedVaccinationMedicalProviderRecord, ContractedVaccinationMedicalProviderSummary, ElderlyWelfareInstitutionRecord, ElderlyWelfareInstitutionSummary, TelepsychologyCounselingInstitutionRecord, TelepsychologyCounselingInstitutionSummary,
} from './types';

const copy = {
  zh: {
    title: '台北公共登記與行政紀錄地圖', subtitle: '人民團體、演藝團體、工會、醫療與心理健康院所、公共意外險、商業異動、公司異動、立案機構、旅宿、旅遊住宿、採購、補助、法規、產業園區、動物照護、兒童照護與老人福利機構公開紀錄探索',
    civicGroups: '人民團體', performingArtsGroups: '演藝團體', vaccinationProviders: '預防接種院所', telepsychology: '通訊心理諮商', publicLiabilityInsurance: '公共意外險', businessChanges: '商業異動', companyChanges: '公司異動', laborUnions: '工會名單', infantCareCenters: '準公共化托嬰中心', elderlyWelfare: '老人福利機構', travelAccommodations: '旅遊住宿', industryGrants: '產業補助廠商', metroProcurement: '捷運採購時程', registeredCramSchools: '立案補習班', registeredHotels: '一般旅館名冊', laborViolations: '勞基法違規公布紀錄', nangangCompanies: '南港軟體工業園區廠商', animalHospitals: '動物醫院一覽表', comparison: '行政區比較',
...
export default function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {tab === 'overview' && summary && performingArtsSummary && vaccinationProviderSummary && telepsychologySummary && publicLiabilitySummary && businessChangeSummary && companyChangeSummary && laborUnionSummary && infantCareSummary && elderlyWelfareSummary && travelAccommodationSummary && grantSummary && procurementSummary && cramSchoolSummary && hotelSummary && laborViolationSummary && nangangCompanySummary && animalHospitalSummary && <CombinedOverview civic={summary} performingArts={performingArtsSummary} vaccinationProviders={vaccinationProviderSummary} telepsychology={telepsychologySummary} publicLiabilityInsurance={publicLiabilitySummary} businessChanges={businessChangeSummary} companyChanges={companyChangeSummary} laborUnions={laborUnionSummary} infantCare={infantCareSummary} elderlyWelfare={elderlyWelfareSummary} travelAccommodations={travelAccommodationSummary} grants={grantSummary} procurement={procurementSummary} cramSchools={cramSchoolSummary} hotels={hotelSummary} laborViolations={laborViolationSummary} nangangCompanies={nangangCompanySummary} animalHospitals={animalHospitalSummary} language={language} />}
      {tab === 'notes' && <section className="workspace notes"><div className="section-heading"><p>09 / METHODOLOGY</p><h2>{t.notes}</h2></div>
        <blockquote>{language === 'zh' ? '本網站整理臺北市公開資料中的人民團體名冊、演藝團體名冊、工會名單、產業補助廠商資料、捷運採購案件預定招標時程、立案補習班資訊、一般旅館名冊、臺北旅遊網住宿資料、勞基法違規公布紀錄、南港軟體工業園區廠商資料、動物醫院一覽表、準公共化托嬰中心、老人福利機構名冊、各項預防接種合約醫療院所、可執行通訊心理諮商之心理機構、營業場所投保公共意外險清冊、商業設立變更及歇業登記異動資料、公司設立變更及解散登記異動資料等公開資料，僅供資料探索與整理使用。各資料集性質不同，不應直接解讀為相同類型組織、活動、營業狀態、法規遵循狀態、信用狀態、投資訊號、法律意見、醫療或長照建議或官方背書。最新與正式資訊請以主管機關正式公告及官方系統為準。' : 'This site organizes Taipei public-data records such as civic group directory records, performing-arts group registry records, labor union directory records, industry grant recipient records, Taipei Metro planned procurement tender schedules, registered cram-school records, registered hotel records, Taipei Travel accommodation records, Labor Standards Act violation publication records, Nangang Software Park company records, animal hospital directory records, quasi-public infant care center records, elderly welfare institution directory records, contracted vaccination medical provider records, telepsychology counseling institution records, business premises public liability insurance records, business establishment / modification / closure registration change records, company establishment / modification / dissolution registration change records, and related public records for data exploration and organization only. These datasets have different meanings and should not be interpreted as the same type of organization, activity, operating status, legal compliance status, credit status, investment signal, legal advice, medical or long-term care advice, or official endorsement. Latest and official information should be verified with official authority notices and official systems.'}</blockquote>
