...
export default function App() {
  ...
  useEffect(() => { const j=(p:string)=>fetch(`${import.meta.env.BASE_URL}${p}`).then(r=>{if(!r.ok)throw Error(p);return r.json()}); Promise.all([j('data/registered-postpartum-care-institutions/records.json'),j('data/out-of-city-funeral-service-businesses/records.json'),j('data/hotel-hygiene-certification-directory/records.json'),j('data/kindergarten-basic-evaluation-pass-records/records.json'),j('data/domestic-employment-service-agencies/records.json')]).then(([a,b,c,d,e])=>{setPostpartumRecords(a);setOutCityFuneralRecords(b);setHotelHygieneRecords(c);setKindergartenRecords(d);setDomesticEmploymentServiceAgencyRecords(e)}).catch(()=>setLoadError(true)); }, []);
