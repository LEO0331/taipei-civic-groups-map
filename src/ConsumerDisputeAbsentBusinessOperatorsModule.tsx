...
export default function ConsumerDisputeAbsentBusinessOperatorsModule({ records, summary, language, related }: { records: ConsumerDisputeAbsentBusinessOperatorRecord[]; summary: ConsumerDisputeAbsentBusinessOperatorSummary; language: Language; related?: { laborViolations?: number; businessChanges?: number; companyChanges?: number; publicLiability?: number } }) {
  ...
  return <><Filters records={records} summary={summary} filters={filters} setFilters={setFilters} language={language} /><DatasetFamilyFrame family={UI_FAMILIES.recordsAnalysis}><DatasetFamilyHeading eyebrow="07 / CONSUMER PROTECTION & LEGAL PUBLIC RECORDS" title={zh ? '消費爭議無故不到場協商之被申訴企業經營者' : 'Consumer Dispute Absent Business Operators'} description={zh ? '整理臺北市依消費者保護自治條例公告之消費爭議協商無故不到場被申訴企業經營者公開資料，包含年度、被申訴人、申訴人、協商日與爭議內容。' : 'Explore Taipei public notices of respondent business operators absent from consumer-dispute negotiation without cause, including year, respondent, complainant, negotiation date, and dispute content.'} />
    <AccessibleTabs tabs={views} value={view} onChange={setView} ariaLabel={zh ? '消費爭議不到場公告資料檢視' : 'Consumer dispute absence data views'} idPrefix="consumer-dispute-absence" /><div className="notice subtle">{notice(zh)}</div><div className="section-heading inline"><div /><strong>{filtered.length.toLocaleString()} <span>{zh ? '筆符合紀錄' : 'matching records'}</span></strong></div>
