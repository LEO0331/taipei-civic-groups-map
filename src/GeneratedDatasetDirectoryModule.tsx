...
export default function GeneratedDatasetDirectoryModule({
  ...
  return <section className="workspace">
    ...
    {view === 'directory' && <div className="comparison-scroll procurement-table"><table><thead><tr>{displayColumns.map(([, label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{filteredRecords.map((record, index) => <tr key={String(record.id ?? index)}>{displayColumns.map(([key]) => <td key={key}>{isMapQueryColumn(key) && record[key] ? <a target="_blank" rel="noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(String(record[key]))}`}>{zh ? '地圖查詢' : 'Map lookup'}</a> : formatValue(record[key])}</td>)}</tr>)}</tbody></table>{!filteredRecords.length && <p className="empty">{zh ? '沒有符合篩選條件的紀錄。' : 'No records match these filters.'}</p>}</div>}
