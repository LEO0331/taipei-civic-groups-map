...
export default function HomeNursingInstitutionsModule({ language }: { language: 'zh' | 'en' }) {
  ...
  if (loadError) return <DatasetFamilyFrame family={UI_FAMILIES.healthcareStandard} className="workspace"><div className="notice">{t('無法載入本機資料檔。', 'The local data file could not be loaded.')}</div></DatasetFamilyFrame>;
  ...
  return <DatasetFamilyFrame family={UI_FAMILIES.healthcareStandard} className="workspace">
    ...
    <AccessibleTabs tabs={[['overview', t('總覽', 'Overview')], ['districts', t('行政區分布', 'District Distribution')], ['directory', t('居家護理所名錄', 'Home Nursing Institution Directory')], ['quality', t('資料品質', 'Data Quality')], ['notes', t('資料說明', 'Data Notes')]] as const} value={view} onChange={(id) => { setView(id); setPage(1); }} ariaLabel={language === 'zh' ? '居家護理所資料檢視' : 'Home nursing institution data views'} idPrefix="home-nursing-institutions" />
