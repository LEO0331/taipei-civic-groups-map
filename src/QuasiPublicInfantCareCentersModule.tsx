...
export default function QuasiPublicInfantCareCentersModule({ records, summary, language }: { records: QuasiPublicInfantCareCenter[]; summary: QuasiPublicInfantCareCenterSummary; language: Language }) {
  ...
  return <><Filters filters={filters} setFilters={setFilters} records={records} language={language} /><DatasetFamilyFrame family={UI_FAMILIES.registryDirectory}><DatasetFamilyHeading eyebrow="10 / CHILDCARE REGISTRY" title={zh ? '準公共化托嬰中心' : 'Quasi-Public Infant Care Centers'} description={zh ? '整理臺北市公開資料中的準公共化托嬰中心名冊，依行政區、地址、核定收托人數、實際收托人數與評鑑結果提供查詢與統計。' : 'Explore Taipei public-data quasi-public infant care center directory records by district, address, approved capacity, actual enrollment, and evaluation result.'} />
    <AccessibleTabs tabs={views} value={view} onChange={setView} ariaLabel={zh ? '準公共化托嬰中心資料檢視' : 'Quasi-public infant care data views'} idPrefix="quasi-public-infant-care" /><div className="notice subtle">{zh ? '準公共化托嬰中心資料為臺北市公開資料中的托嬰中心名冊，僅供資料查詢、行政區分布、核定收托人數、實際收托人數與評鑑結果整理使用，不代表即時收托名額、服務品質保證、推薦排名、托育建議、收費資訊、營運狀態或官方背書。實際收托名額、收費、服務內容、營運狀態與最新評鑑資訊請以托嬰中心、主管機關或官方公告為準。' : 'Quasi-public infant care center data is a Taipei public-data directory of infant care centers for lookup, district distribution, approved capacity, actual enrollment, and evaluation-result organization only. It does not represent real-time vacancy, service-quality guarantee, recommendation ranking, childcare advice, pricing information, operating status, or official endorsement. Actual vacancies, fees, services, operating status, and latest evaluation information should be verified with the infant care center, competent authority, or official announcements.'}</div>
