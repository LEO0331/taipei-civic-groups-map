...
import { cleanText, parseHealthcareProviderPhone } from './contractedVaccinationMedicalProviders';
import type { DentureSubsidyMedicalProviderFilters, DentureSubsidyMedicalProviderRecord, DentureSubsidyMedicalProviderSummary, MedicalProviderPhoneType } from '../types';

export const normalizeText = (raw: unknown) => cleanText(raw)?.toLocaleLowerCase();

export function parseDentureSubsidyType(raw: unknown) {
  const subsidyTypeRaw = cleanText(raw);
  const subsidyType = subsidyTypeRaw;
  const subsidyTypeNormalized = normalizeText(subsidyTypeRaw);
  return { subsidyTypeRaw, subsidyType, subsidyTypeNormalized, warning: subsidyTypeRaw && subsidyTypeRaw.includes('假牙補助') ? undefined : 'Unexpected subsidy type' };
}
...
export const normalizeMedicalProviderName = (raw: unknown) => cleanText(raw)?.replace(/\s+/g, ' ').toLocaleLowerCase();
...
export function createDentureSubsidyProviderMapQuery(record: { providerName?: string; address?: string }) {
...
export function buildDentureSubsidyMedicalProviderSummary(records: DentureSubsidyMedicalProviderRecord[]): DentureSubsidyMedicalProviderSummary {
  const areas = countBy(records.flatMap((record) => record.areaDisplayName ?? record.administrativeArea ?? []));
  const districts = countBy(records.flatMap((record) => record.taipeiDistrict ?? []));
  const subsidyTypes = countBy(records.flatMap((record) => record.subsidyType ?? []));
  return {
    ...
    subsidyTypeCount: subsidyTypes.length,
    ...
    administrativeAreaCount: areas.length,
    ...
    bySubsidyType: subsidyTypes.map(({ key: subsidyType, count }) => ({ subsidyType, providerCount: count })),
    byAdministrativeArea: areas.map(({ key: administrativeArea, count }) => {
    ...
    byTaipeiDistrict: districts.map(({ key: district, count }) => {
    ...
    dataQuality: {
      duplicateProviderNameCount: duplicateCount(records.map((record) => record.providerNameNormalized)),
...
export function filterDentureSubsidyMedicalProviders(records: DentureSubsidyMedicalProviderRecord[], filters: DentureSubsidyMedicalProviderFilters) {
  const query = filters.search.trim().toLocaleLowerCase();
  return records.filter((record) => (!query || [record.subsidyType, record.providerName, record.administrativeArea, record.address, record.roadName, record.phone].filter(Boolean).join(' ').toLocaleLowerCase().includes(query))
    && (!filters.subsidyType || record.subsidyType === filters.subsidyType)
